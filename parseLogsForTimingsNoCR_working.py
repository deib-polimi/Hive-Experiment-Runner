# This will parse the logs for a single application, collecting info about main time intervals
# and writing on several files info for the subsequent dag creation (we do this for every
# application to be able to use this info also for further analysis)
import sys
import os
import re
try:
...
def getTime(some):
	h = int(some.group(1))*60*60*1000
	m = int(some.group(2))*60*1000
	s = int(some.group(3))*1000
	ms = int(some.group(4))
	return (h+m+s+ms)
	
	
rmLog = open(os.path.join(sys.path[0], "wd/RMLOG.txt"),"r")
amLog = open(os.path.join(sys.path[0], "wd/AMLOG.txt"),"r")
fc = open(os.path.join(sys.path[0], "wd/firstContainerInfo.txt"),"a")
rc = open(os.path.join(sys.path[0], "wd/releaseContainer.txt"),"a")
gc = open(os.path.join(sys.path[0], "wd/genericContainerStartup.txt"),"a")
ca = open(os.path.join(sys.path[0], "wd/containerAcquisition.txt"),"a")
#DAG specific
al = open(os.path.join(sys.path[0], "wd/appsList.txt"),"a")
vo = open(os.path.join(sys.path[0], "wd/vertexOrder.txt"),"a")
vltk = open(os.path.join(sys.path[0], "wd/vertexLtask.txt"),"a")
vpri = open(os.path.join(sys.path[0], "wd/vertexPriority.txt"),"a")
otk = open(os.path.join(sys.path[0], "wd/taskLaunchOrder.txt"),"a")
tdlo = open(os.path.join(sys.path[0], "wd/taskDurationLO.txt"),"a")

isOldMapping=False
if os.path.isfile(os.path.join(sys.path[0], "wd/vmap.bin")):
	vmap = open(os.path.join(sys.path[0], "wd/vmap.bin"),"r")
	print "Found mapping vname-vid, using this."
	map_vid_vname=pickle.loads(vmap.read())
	isOldMapping=True
else:
	vmap = open(os.path.join(sys.path[0], "wd/vmap.bin"),"w")
...
while line:
	...
	# Case: release container because new container and no task, AM side. Only case when an unused container is released.
	...
	if "EPILOGUE" in first_container_startup_status:
		found = re.search(r'.+app.DAGAppMaster: Exiting DAGAppMaster..GoodBye!', line)
		if not found:
			found = re.search(r'.+recovery.RecoveryService: Error when closing summary stream', line)
		if found:
...
for cnt in release_am_side.keys():
	...
	if cnt in container_type.keys():
		...
		if interval<200 and interval > 0:
			print "Found deallocated container 'END' with release time lower than 200: "+cnt
			print "Please check current saved logs."
			#exit(-1)
		elif interval < 1:
			print "Found release time < 0: "+cnt
